...
abstract class WorkerTrackingDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_24_25 = object : Migration(24, 25) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Add isReferenceHourlyRate column to support fixed amount commissions for reference workers
                // Default to 1 (true) to maintain existing behavior (hourly commission)
                db.execSQL("ALTER TABLE event_workers ADD COLUMN isReferenceHourlyRate INTEGER NOT NULL DEFAULT 1")
                db.execSQL("ALTER TABLE shift_workers ADD COLUMN isReferenceHourlyRate INTEGER NOT NULL DEFAULT 1")
            }
        }
